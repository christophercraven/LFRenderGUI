...
LFROOT = "" # THE MOST IMPORTANT VARIABLE
outname = ""
outdir = ""
file = None
meshfile = None
...
def ExportIt(selectedList=None):
    ...
    if BFREAD_OK and Tblendimp.val:
        ...
        try:
            ...
            raise "Error in blendfile import"
        else:
    ...
    if BL_VERSION>223:
        ...
        if cameraObj==None:
            raise "No active scene camera found, add a camera"
        tp = cameraObj.getType()
        if tp!='Camera':
            raise "Camera object is a %s! Use alt-0 to reset the camera" % tp
        ...
        if cameraObj.type!='Camera':
            raise "Camera object is a %s! Use alt-0 to reset the camera" % cameraObj.type
...
def GetWindowFactors():
    ...
    return wsize[2]*0.5, wsize[2]/393.0, wsize[3]/510.0
...
## jms : modif 16/08/2003
        ...
        doc_path = None
        if Tdocmenu.val==1:
            doc_path = os.path.normpath(os.path.join(os.environ['LIGHTFLOWPATH'], 'PM/Docs/ClassDoc.html'))
        elif Tdocmenu.val!=0:   # 0 = (pseudo)title
            # see if the 'LFE_DOCS' directory is there, if not warn the user it is not there
            doc_path = os.path.normpath(os.path.join(os.environ['LIGHTFLOWPATH'], 'PM/LFE_DOCS'))
...
def LoadRawTGA():
    ...
    try:
        ...
        try:
            ...
            if img_type!=2:
                raise "Unsupported Targa image format!"
...
def pt_draw():
    ...
    Tpath = Draw.String("Path: ", evt_ignore, Lstart, 455*muly, 375*mulx, 18*muly, Tpath.val, 128,
...
def pt_bevent(evt):
    global PATH_OK, LFROOT, LFE_scene_filepath, MSP_PATHOK, MSP_PATH, TEXROOT
    if evt == evt_save:
        ...
        PATH_OK = [1, '']
        if not os.path.exists(LFROOT):
            try:
                os.mkdir(LFROOT)
            except OSError:
                PATH_OK = [0, ("Can't create directory with name '%s'" % LFROOT)]
                Blender.Redraw()
            else:
                print "%s directory created" % LFROOT
        else:
            print "%s already exists" % LFROOT

        if PATH_OK[0]:
            # SAVE THE PATH SETTINGS IN THE LIGHTFLOW DIRECTORY
            pfilename = os.path.join(os.environ['LIGHTFLOWPATH'], LFE_paths_filename)
            try:
        ...
        fname = os.path.join(os.environ['LIGHTFLOWPATH'], 'PM/LFE_DOCS/GeneralInfo.html')
        BROWSER_OPEN(fname)
    elif evt == evt_tested:
...
def GetOutdir():
    ...
    outdir = os.path.join(LFROOT, outname)
...
def WriteDirsSettings():
    ...
    if not os.path.exists(outdir):
        try:
            ...
            raise "Could not create file output directory"
        else:
    ...
    if not os.path.exists(imgdir):
        try:
            ...
            raise "Could not create image output directory"
        else:
    ...
    if not os.path.exists(tgatex_dir):
        try:
            ...
            raise "Could not create texture output directory"
        else:
    ...
    if not os.path.exists(alldata_dir):
        try:
            ...
            raise "Could not create data output directory"
        else:
    ...
    if not WRITE_SETTINGS(): raise "Settingsfile write error"
    ...
    if sys.platform=='win32':
        ...
        if os.path.exists(tpy):
            ...
            if os.path.exists(tpy):
                ...
                raise "Sorry, can't find python binary path..."
...
def START():
    global PATH_TITLE, PATH_OK, Tpath, LFROOT, Teditpath, Tmsp_path, MSP_PATH, Ttex_dir, TEXROOT, Timsi_pref, Tpy_execpath
    global Tbrowser
